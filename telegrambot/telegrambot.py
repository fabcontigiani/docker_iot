...
async def sin_autorizacion(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.text == password:
        ...
        await context.bot.send_message(
            ...
            text="Contraseña correcta, ahora está autorizado",
        )
    else:
        logging.info("no autorizado: " + str(update.message.from_user.id))
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="Usuario no autorizado, escriba la contraseña para autorizarse",
        )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.from_user.last_name:
        ...
        apellido = ""
    await context.bot.send_message(
        update.message.chat.id, text="Bienvenido al Bot " + nombre + " " + apellido
    )
...
async def acercade(update: Update, context):
    await context.bot.send_message(
        update.message.chat.id, text="Este bot fue creado para el curso de IoT FIO"
    )


async def configurar(update: Update, context):
    await update.message.reply_text(
        "Iniciando configuración, escriba 'cancelar' para salir.\nElija una opción:",
        reply_markup=ReplyKeyboardMarkup(
            [["setpoint", "modo"], ["periodo", "rele"]],
...
async def eligiendo(update: Update, context):
    ...
    if "setpoint" in update.message.text:
        ...
        await update.message.reply_text(
            "Ingrese el nuevo setpoint (float)", reply_markup=ReplyKeyboardRemove()
        )
        return SETPOINT
        ...
        await update.message.reply_text(
            "Ingrese el nuevo periodo (segundos, entero)",
        ...
        await update.message.reply_text(
            "Ingrese el nuevo estado del rele",
            reply_markup=ReplyKeyboardMarkup(
...
async def destello(update: Update, context):
    await context.bot_data["mqtt_client"].publish(
        pico_id + "/destello", '{"destello": 1}'
    )
...
async def setpoint(update: Update, context):
    ...
    try:
        ...
        await context.bot_data["mqtt_client"].publish(
            pico_id + "/setpoint", f'{{"setpoint": {setpoint}}}'
        )
...
async def modo(update: Update, context):
    ...
    if "automático" in modo:
        await context.bot_data["mqtt_client"].publish(
            pico_id + "/modo", '{"modo": "automatico"}'
        )
        await update.message.reply_text(
            f"Modo configurado a {modo}", reply_markup=ReplyKeyboardRemove()
        )
    elif "manual" in modo:
        await context.bot_data["mqtt_client"].publish(
            pico_id + "/modo", '{"modo": "manual"}'
        )
        await update.message.reply_text(
            f"Modo configurado a {modo}", reply_markup=ReplyKeyboardRemove()
        )
        ...
        await update.message.reply_text(
            "Error: modo no válido.", reply_markup=ReplyKeyboardRemove()
        )
    return ConversationHandler.END
...
async def periodo(update: Update, context):
    ...
    if periodo.isdigit():
        await context.bot_data["mqtt_client"].publish(
            pico_id + "/periodo", f'{{"periodo": {periodo}}}'
        )
...
async def rele(update: Update, context):
    ...
    if "cerrado" in rele:
        ...
        await update.message.reply_text(
            f"Rele configurado a {rele}", reply_markup=ReplyKeyboardRemove()
        )
        ...
        await update.message.reply_text(
            "Error: estado de rele no válido.", reply_markup=ReplyKeyboardRemove()
        )
    return ConversationHandler.END
...
async def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ELIGIENDO: [
                MessageHandler(
                    filters.TEXT & ~(filters.COMMAND | filters.Regex("^cancelar$")),
                    eligiendo,
                )
            ],
            SETPOINT: [
